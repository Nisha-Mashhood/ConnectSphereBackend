...
ContactMessageSchema.pre("save", async function (next) {
    if (!this.contactMessageId) {
        try {
            this.contactMessageId = await (0, id_generator_1.generateCustomId)("contactMessage", "CNM");
            logger_1.default.debug(`Generated contactMessageId: ${this.contactMessageId} for name ${this.name}`);
        }
        catch (error) {
            logger_1.default.error(`Error generating contactMessageId: ${this.contactMessageId} for name ${this.name} : ${error}`);
            return next(error);
        }
    }
    next();
