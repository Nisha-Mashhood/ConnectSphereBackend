"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.CallSocketHandler = void 0;
const logger_1 = __importDefault(require("../core/utils/logger"));
const call_log_helper_1 = require("./Utils/call-log-helper");
const inversify_1 = require("inversify");
let CallSocketHandler = class CallSocketHandler {
    constructor(contactsRepo, groupRepo, userRepo, notificationService, callLogRepo) {
        this._activeOffers = new Map();
        this._endedCalls = new Set();
        this._io = null;
        this._contactsRepo = contactsRepo;
        this._groupRepo = groupRepo;
        this._userRepo = userRepo;
        this._notificationService = notificationService;
        this._callLogRepo = callLogRepo;
        logger_1.default.debug(`CallSocketHandler initialized with callLogRepo: ${!!callLogRepo}`);
        if (!callLogRepo) {
            logger_1.default.error("CallLogRepository is not initialized");
            throw new Error("CallLogRepository is required");
        }
    }
    setIo(io) {
        this._io = io;
    }
    async handleOffer(socket, data) {
        try {
            const { userId, targetId, type, chatKey, offer, callType } = data;
            logger_1.default.info(`Received ${callType} offer from ${userId} for chatKey: ${chatKey}`);
            let room;
            let recipientIds = [];
            let contentType;
            if (type === "group") {
                // group call path (we are not implementing full group flow yet, but keep logic consistent)
                room = `group_${targetId}`;
                contentType = "group";
                const group = await this._groupRepo.getGroupById(targetId);
                if (!group) {
                    logger_1.default.error(`Invalid group ID: ${targetId}`);
                    socket.emit("error", { message: "Invalid group ID" });
                    return;
                }
                recipientIds = group.members
                    .filter((member) => member.userId.toString() !== userId)
                    .map((member) => member.userId.toString());
            }
            else {
                // one-on-one (user-user or user-mentor)
                const contact = await this._contactsRepo.findContactByUsers(userId, targetId);
                if (!contact) {
                    logger_1.default.error(`Invalid contact for offer: userId=${userId}, targetId=${targetId}`);
                    socket.emit("error", { message: "Invalid contact" });
                    return;
                }
                const ids = [contact.userId.toString(), contact.targetUserId?.toString()].sort();
                room = `chat_${ids[0]}_${ids[1]}`;
                recipientIds = [targetId];
                contentType = contact.type === "user-mentor" ? "collaboration" : "userconnection";
            }
            const callId = `${chatKey}_${Date.now()}`;
            // Broadcast the offer to the room (recipient(s) will receive it)
            socket.broadcast.to(room).emit("offer", {
                userId,
                targetId,
                type,
                chatKey,
                offer,
                callType,
                callId,
                senderName: (await this._userRepo.findById(userId))?.name || "Unknown",
            });
            // Create server-side callId and call log
            const sender = await this._userRepo.findById(userId);
            const callLog = await (0, call_log_helper_1.createCallLog)(socket, this._io, this._callLogRepo, {
                CallId: callId,
                chatKey,
                callType,
                type,
                senderId: userId,
                recipientIds,
                groupId: type === "group" ? targetId : undefined,
                callerName: sender?.name || "Unknown",
            });
            if (!callLog) {
                logger_1.default.error(`Failed to create call log for callId: ${callId}`);
                socket.emit("error", { message: "Failed to create call record" });
                return;
            }
            // Notify the offerer (the socket that sent the offer) with server-generated callId and metadata
            const createdEvent = {
                callId,
                chatKey,
                callType,
                type,
                senderId: userId,
                recipientIds,
                groupId: type === "group" ? targetId : undefined,
                callerName: sender?.name || "Unknown",
                createdAt: new Date().toISOString(),
            };
            socket.emit("call.created", createdEvent);
            logger_1.default.info(`Emitted call.created to offerer ${userId} with callId ${callId}`);
            // send incoming call notifications for each recipient (non-blocking)
            for (const recipientId of recipientIds) {
                try {
                    const notification = await this._notificationService.sendNotification(recipientId, "incoming_call", userId, chatKey, contentType, callId, callType, `incoming ${callType} call from ${sender?.name || "Unknown"}`);
                    if (notification && this._io) {
                        // if the recipient is connected, notify them directly
                        this._io.to(`user_${recipientId}`).emit("notification.new", notification);
                    }
                    logger_1.default.info(`Created call notification for user ${recipientId}: ${notification?.id}`);
                }
                catch (err) {
                    logger_1.default.warn(`Failed to send call notification to user ${recipientId}: ${err.message}`);
                }
            }
            // create end-timeout for unanswered call (30s)
            const endTimeout = setTimeout(async () => {
                const call = this._activeOffers.get(callId);
                if (!call)
                    return;
                // find sockets currently connected in the room to determine who answered
                const socketsInRoom = await this._io?.in(room).allSockets();
                const connectedUserIds = new Set();
                if (socketsInRoom) {
                    for (const socketId of socketsInRoom) {
                        const s = this._io?.sockets.sockets.get(socketId);
                        if (s && s.data.userId) {
                            connectedUserIds.add(s.data.userId);
                        }
                    }
                }
                // for each recipient, if they never connected, mark call as missed for that recipient
                for (const recipientId of recipientIds) {
                    if (!connectedUserIds.has(recipientId)) {
                        const updatedCallLog = await (0, call_log_helper_1.updateCallLog)(socket, this._io, this._callLogRepo, callId, userId, recipientIds, {
                            status: "missed",
                            endTime: new Date(),
                        });
                        if (!updatedCallLog) {
                            logger_1.default.error(`Failed to update call log to missed for CallId: ${callId}`);
                            continue;
                        }
                        // update notification to missed or create missed notification
                        const notification = await this._notificationService.updateCallNotificationToMissed(recipientId, callId, `Missed ${callType} call from ${userId}`);
                        if (notification && this._io) {
                            this._io.to(`user_${recipientId}`).emit("notification.updated", notification);
                        }
                        else {
                            // fallback: create missed notification
                            const newNotification = await this._notificationService.sendNotification(recipientId, "missed_call", userId, chatKey, contentType, callId, callType, `Missed ${callType} call from ${sender?.name || "Unknown"}`);
                            if (this._io && newNotification) {
                                this._io.to(`user_${recipientId}`).emit("notification.new", newNotification);
                            }
                        }
                    }
                }
                // inform all sockets in the room that call ended (missed)
                socket.to(room).emit("callEnded", { userId, targetId, type, chatKey, callType, callId, reason: "missed" });
                socket.emit("callEnded", { userId, targetId, type, chatKey, callType, callId, reason: "missed" });
                // cleanup
                this._activeOffers.delete(callId);
                logger_1.default.info(`Call ${callId} marked missed and cleaned up after timeout`);
            }, 30000);
            // store active offer with endTimeout
            const callOffer = {
                senderId: userId,
                targetId,
                type,
                chatKey,
                callType,
                recipientIds,
                endTimeout,
            };
            this._activeOffers.set(callId, callOffer);
            logger_1.default.info(`Stored active offer for callId ${callId}`);
        }
        catch (error) {
            logger_1.default.error(`Error broadcasting offer: ${error.message}`);
            socket.emit("error", { message: "Failed to send offer" });
        }
    }
    async handleAnswer(socket, data) {
        try {
            const { userId, targetId, type, chatKey, answer, callType } = data;
            logger_1.default.info(`Received ${callType} answer from ${userId} for chatKey: ${chatKey}`);
            let room;
            if (type === "group") {
                room = `group_${targetId}`;
            }
            else {
                const contact = await this._contactsRepo.findContactByUsers(userId, targetId);
                if (!contact) {
                    logger_1.default.error(`Invalid contact for answer: userId=${userId}, targetId=${targetId}`);
                    socket.emit("error", { message: "Invalid contact" });
                    return;
                }
                const ids = [
                    contact.userId.toString(),
                    contact.targetUserId?.toString(),
                ].sort();
                room = `chat_${ids[0]}_${ids[1]}`;
            }
            socket.broadcast
                .to(room)
                .emit("answer", { userId, targetId, type, chatKey, answer, callType });
            const callId = Array.from(this._activeOffers.keys()).find((id) => this._activeOffers.get(id)?.chatKey === chatKey &&
                this._activeOffers.get(id)?.senderId === targetId);
            if (callId) {
                const call = this._activeOffers.get(callId);
                if (call) {
                    clearTimeout(call.endTimeout);
                    this._activeOffers.delete(callId);
                }
            }
        }
        catch (error) {
            logger_1.default.error(`Error broadcasting answer: ${error.message}`);
            socket.emit("error", { message: "Failed to send answer" });
        }
    }
    async handleIceCandidate(socket, data) {
        try {
            const { userId, targetId, type, chatKey, candidate, callType } = data;
            logger_1.default.info(`Received ${callType} ICE candidate from ${userId} for chatKey: ${chatKey}`);
            let room;
            if (type === "group") {
                room = `group_${targetId}`;
            }
            else {
                const contact = await this._contactsRepo.findContactByUsers(userId, targetId);
                if (!contact) {
                    logger_1.default.error(`Invalid contact for ICE candidate: userId=${userId}, targetId=${targetId}`);
                    socket.emit("error", { message: "Invalid contact" });
                    return;
                }
                const ids = [
                    contact.userId.toString(),
                    contact.targetUserId?.toString(),
                ].sort();
                room = `chat_${ids[0]}_${ids[1]}`;
            }
            socket.broadcast.to(room).emit("ice-candidate", {
                userId,
                targetId,
                type,
                chatKey,
                candidate,
                callType,
            });
        }
        catch (error) {
            logger_1.default.error(`Error broadcasting ICE candidate: ${error.message}`);
            socket.emit("error", { message: "Failed to send ICE candidate" });
        }
    }
    async handleCallEnded(socket, data) {
        try {
            const { userId, targetId, type, chatKey, callType } = data;
            const callId = Array.from(this._activeOffers.keys()).find((id) => this._activeOffers.get(id)?.chatKey === chatKey &&
                this._activeOffers.get(id)?.senderId === userId);
            if (!callId || this._endedCalls.has(callId)) {
                logger_1.default.info(`Ignoring duplicate or invalid callEnded for callId: ${callId}, chatKey: ${chatKey}`);
                return;
            }
            logger_1.default.info(`Received callEnded from ${userId} for chatKey: ${chatKey}, callType: ${callType}`);
            let room;
            let recipientIds = [];
            if (type === "group") {
                room = `group_${targetId}`;
                const group = await this._groupRepo.getGroupById(targetId);
                if (!group) {
                    logger_1.default.error(`Invalid group ID: ${targetId}`);
                    socket.emit("error", { message: "Invalid group ID" });
                    return;
                }
                recipientIds = group.members
                    .filter((member) => member.userId.toString() !== userId)
                    .map((member) => member.userId.toString());
            }
            else {
                const contact = await this._contactsRepo.findContactByUsers(userId, targetId);
                if (!contact) {
                    logger_1.default.error(`Invalid contact for callEnded: userId=${userId}, targetId=${targetId}`);
                    socket.emit("error", { message: "Invalid contact" });
                    return;
                }
                const ids = [
                    contact.userId.toString(),
                    contact.targetUserId?.toString(),
                ].sort();
                room = `chat_${ids[0]}_${ids[1]}`;
                recipientIds = [targetId];
            }
            // Update call log
            const updatedCallLog = await (0, call_log_helper_1.updateCallLog)(socket, this._io, this._callLogRepo, callId, userId, recipientIds, {
                status: "completed",
                endTime: new Date(),
            });
            if (!updatedCallLog) {
                logger_1.default.error(`Failed to update call log to completed for CallId: ${callId}`);
                return;
            }
            this._io
                ?.to(room)
                .emit("callEnded", { userId, targetId, type, chatKey, callType });
            this._endedCalls.add(callId);
            setTimeout(() => this._endedCalls.delete(callId), 60000);
            const call = this._activeOffers.get(callId);
            if (call) {
                clearTimeout(call.endTimeout);
                this._activeOffers.delete(callId);
            }
        }
        catch (error) {
            logger_1.default.error(`Error handling callEnded: ${error.message}`);
            socket.emit("error", { message: "Failed to end call" });
        }
    }
};
exports.CallSocketHandler = CallSocketHandler;
exports.CallSocketHandler = CallSocketHandler = __decorate([
    (0, inversify_1.injectable)(),
    __param(0, (0, inversify_1.inject)("IContactRepository")),
    __param(1, (0, inversify_1.inject)("IGroupRepository")),
    __param(2, (0, inversify_1.inject)("IUserRepository")),
    __param(3, (0, inversify_1.inject)("INotificationService")),
    __param(4, (0, inversify_1.inject)("ICallLogRepository")),
    __metadata("design:paramtypes", [Object, Object, Object, Object, Object])
], CallSocketHandler);
//# sourceMappingURL=call-socket-handler.js.map